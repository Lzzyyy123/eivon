...
import { ArrowRight, Boxes, Bot, ChartNoAxesCombined, Clock3, Eye, EyeOff, FlaskConical, Languages, LayoutDashboard, LibraryBig, LogOut, MessagesSquare, Settings2, UsersRound, Workflow } from "lucide-react";
...
function AuthCard({ mode, onComplete }: { mode: "login" | "setup"; onComplete: (value: Identity) => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const result = await api<Identity>(mode === "setup" ? "/setup" : "/auth/login", { method: "POST", body: JSON.stringify(mode === "setup" ? form : { email: form.email, password: form.password }) });
      onComplete(result);
    } catch (e) { setError((e as Error).message); }
    finally { setBusy(false); }
  }
  return <main className="auth-layout">
    <div className="auth-brand-panel"><BrandMark /><div className="auth-brand-bottom"><span>{t("AGENT WORKBENCH")}</span><strong>EIVON</strong></div></div>
    <div className="auth-content">
      <div className="auth-topbar"><div className="auth-mobile-brand"><BrandMark compact /></div><LanguageSwitch /></div>
      <form className="auth-form" onSubmit={submit}>
        <div className="auth-intro"><span className="eyebrow">{t(mode === "setup" ? "INITIAL SETUP" : "ACCOUNT ACCESS")}</span><h1>{t(mode === "setup" ? "Create your workspace" : "Welcome back")}</h1><p>{t(mode === "setup" ? "Set up the first administrator for this self-hosted instance." : "Sign in to manage your agents.")}</p></div>
        {error && <div className="notice error" role="alert">{error}</div>}
        <div className="auth-form-fields">
          {mode === "setup" && <><label>{t("Setup token")}<input name="setup_token" required autoComplete="off" value={form.setup_token} onChange={(e) => setForm({ ...form, setup_token: e.target.value })} /></label><label>{t("Your name")}<input name="name" required autoComplete="name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label><label>{t("Workspace name")}<input name="workspace_name" required autoComplete="organization" value={form.workspace_name} onChange={(e) => setForm({ ...form, workspace_name: e.target.value })} /></label></>}
          <label>{t("Email")}<input name="email" required type="email" autoComplete="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></label>
          <label>{t("Password")}<span className="auth-password"><input name="password" required type={showPassword ? "text" : "password"} autoComplete={mode === "setup" ? "new-password" : "current-password"} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /><button type="button" aria-label={t(showPassword ? "Hide password" : "Show password")} title={t(showPassword ? "Hide password" : "Show password")} onClick={() => setShowPassword(!showPassword)}>{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></span></label>
        </div>
        <Button type="submit" disabled={busy}>{t(busy ? (mode === "setup" ? "Initializing…" : "Signing in…") : (mode === "setup" ? "Initialize Eivon" : "Sign in"))}<ArrowRight size={16} aria-hidden="true" /></Button>
      </form>
      <div className="auth-footer">EIVON <span>·</span> {t("Self-hosted agent workbench")}</div>
    </div>
  </main>;
}
